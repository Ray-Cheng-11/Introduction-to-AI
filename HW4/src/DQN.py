...
class Agent:
    ...
    def learn(self):
        ...
        # Begin your code
        """
        Sample trajectories of batch size from the replay buffer. Use function "sample" defined in class "replay_buffer" to get the sampled data. Then, convert these sampled data into tensor.
        """
        ...
        """
        Forward the data to the evaluate net and the target net. Variable "q_value" is predicted values from evaluate network which is extracted based on "action". Variable "q_next_value" is actual values from target network. Variable "target_q_value" is the expected Q-values obtained from the formula "reward + gamma * max(q_next_value)".
        """
        q_value = self.evaluate_net(states).gather(1, actions)
        q_next_value = self.target_net(next_states).detach() * (~done).unsqueeze(-1)
        target_q_value = rewards.unsqueeze(-1) + self.gamma * q_next_value.max(1)[0].view(self.batch_size, 1)
        ...
        """
        Zero-out the gradients before doing back propagation, or the gradient would be a combination of 
    ...
    def choose_action(self, state):
        ...
        with torch.no_grad():
            # Begin your code
            """
            Generate a random number which interval is between [0, 1]. If the number is bigger than epsilon, determine to exploit and return the index of the maximum Q of the given state forwarded by the evaluate neural network; otherwise, determine to explore and return random action.
            """
