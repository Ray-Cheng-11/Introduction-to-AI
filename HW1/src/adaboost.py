...
class Adaboost:
    ...
    def selectBest(self, featureVals, iis, labels, features, weights):
        ...
        '''
        Take t_pos and t_neg to accumulate the total weights of positive and negative samples. 
        Iterates over each label and its corresponding weight, incrementing t_pos for positive labels and t_neg for negative labels. 
        Then, iterating through each feature and its values, sorting the samples based on their values. 
        For each feature, calculating the error considering the current feature, and updates the best error, best feature, best threshold, 
        and best polarity if a smaller error is found. Also keeps track of the cumulative positive and negative weights and their counts. 
        Finally, creating a WeakClassifier object with the best feature, threshold, and polarity, 
        representing the best weak classifier based on the given features, feature values, labels, and weights.
        '''
        ...
        for feature, vals in zip(features, featureVals):
            ...
            applied = sorted(zip( weights, vals, labels), key= lambda x: x[1])
            cur_pos_weight, cur_neg_weight, cur_pos_num, cur_neg_num = 0, 0, 0, 0

            for w, val, label in applied:

                error = min(cur_pos_weight + t_neg - cur_neg_weight,
                             cur_neg_weight + t_pos - cur_pos_weight)
